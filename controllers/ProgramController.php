...
class ProgramController extends Controller
{
    ...
    public function actionViewRubric($id, $edit = null, $cat = null){
        ...
        if($prs){
            foreach($prs as $pr){
                $role = $this->findManagerRole($pr->program_id, $pr->program_sub);
                if($role){
    ...
    public function actionAchievement($id, $sub = null){
        ...
        if(Yii::$app->request->isPost){
            ...
            if($action === 'add'){
                $achievementModel = new ProgramAchievement([
                    'program_id' => (int)$id,
                    'program_sub' => $programSub ? (int)$programSub->id : null,
                    'name' => $name,
                ]);
            ...
            if($action === 'update' || $action === 'delete'){
                ...
                if($action === 'update'){
                    ...
                    $usedCount = ParticipantAchieve::find()->where(['achieve_id' => $achievementModel->id])->count();
                    if($usedCount > 0){
                        Yii::$app->session->addFlash('error', 'Achievement cannot be deleted because it is already used.');
                    }else if($achievementModel->delete()){
        ...
        if($programSub){
            ...
            ->where(['program_id' => $id, 'program_sub' => $sub])->orderBy(['name' => SORT_ASC])->all();
        }else{
    ...
    protected function findManagerRole($id, $sub = null)
    {
        ...
        if($sub){
            $query->andWhere(['or', ['program_sub' => $sub], ['program_sub' => null]]);
        }else{
            $query->andWhere(['program_sub' => null]);
        }
    ...
    public function actionCertificate() //program
    {
        ...
        $achievements = ProgramRegistration::find()->alias('a')
        ->joinWith(['program p'])
        ->with(['achievements.achieve', 'achievements.winnerTitle'])
        ->innerJoin('program_reg_achieve pra', 'pra.program_reg_id = a.id')
    ...
    protected function findRegistrationAchievement($id, $achievementId = null)
    {
        ...
        ->innerJoin('program_reg_achieve pra', 'pra.program_reg_id = a.id')
        ->where(['a.id' => $id])
        ->andFilterWhere(['pra.achieve_id' => $achievementId])
        ->one();
        if ($model !== null) {
            return $model;
        }
    ...
    public function actionCertParticipation($reg){
        ...
        if($reg->user_id == Yii::$app->user->identity->id || Yii::$app->user->identity->isManager || Yii::$app->user->identity->isAdmin || $mentor){ // atau mentor
    ...
    public function actionCertParticipationSession($reg, $s, $u){
        ...
        if($session->user_id == Yii::$app->user->identity->id || Yii::$app->user->identity->isManager || Yii::$app->user->identity->isAdmin){
    ...
    public function actionCertAchievement($reg, $achieve = null){
        if(!$this->ensureCertificatesReleased(true, 4)){
            return $this->render('empty');
        }

        $pdf = new CertificateAchievement;
        $reg = $this->findRegistrationAchievement($reg, $achieve);
        $mentor = $this->meAsMentor($reg);
        if($reg->user_id == Yii::$app->user->identity->id || Yii::$app->user->identity->isManager || Yii::$app->user->identity->isAdmin || $mentor){
            ...
            $pdf->model = $reg;
            $pdf->achievementId = $achieve ? (int)$achieve : null;
            $pdf->generatePdf();
        }
        exit;
    }
